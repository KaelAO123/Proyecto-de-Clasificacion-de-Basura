            ...
            if class_path.exists():
                ...
                if images:
                    ...
                    if img is not None:
                        ...
                        print(f"  ⚠️ {class_name}: {len(images)} imágenes - Error al leer ejemplo")
                else:
    ...
    def _manual_split_dataset(self, train_ratio, val_ratio, test_ratio):
        ...
        for class_name in self.config['data']['classes']:
            ...
            if not class_path.exists():
                print(f"  ⚠️ Saltando {class_name}: directorio no existe")
                continue
                
            images = list(class_path.glob("*.jpg")) + list(class_path.glob("*.jpeg")) + list(class_path.glob("*.png"))
            if not images:
                print(f"  ⚠️ Saltando {class_name}: sin imágenes")
                continue
